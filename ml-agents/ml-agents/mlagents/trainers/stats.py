...
class ConsoleWriter(StatsWriter):
    ...
    def write_stats(
        ...
        if "Policy/Curiosity Reward" in values:
            stats_summary = values["Policy/Curiosity Reward"]
            log_info.append(f"Mean Curiosity Reward: {stats_summary.mean:0.3f}")

        if "Losses/Curiosity Forward Loss" in values:
            stats_summary = values["Losses/Curiosity Forward Loss"]
            log_info.append(f"Mean Forward Loss: {stats_summary.mean:0.3f}")

        if "Losses/Curiosity Inverse Loss" in values:
